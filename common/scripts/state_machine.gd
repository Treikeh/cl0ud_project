...
func _init(states: Dictionary[int, Dictionary]) -> void:
	_states = states


func switch(new_state: int) -> void:
	var old_state: Dictionary = _current
	# Check if state exists before entering the new state
	if not _states.has(new_state):
		return
	
	_current = _states[new_state]# if _states.has(new) else {}
	
	if _current != old_state:
		if old_state.has(EXIT):
			old_state[EXIT].call()
